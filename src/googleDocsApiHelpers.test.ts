...
import {
  ...
  getParagraphRange,
  mergeTableCells,
  updateTableCellBackgroundColor,
...
describe('Table Cell Mutations', () => {
  function buildMockDocsForTableMutations(tableStartIndex: number, tableRows: unknown[][]) {
    return {
      documents: {
        get: vi.fn(async () => ({
          data: {
  ...
  it('should create a mergeTableCells request for a valid range', async () => {
    const mockDocs = buildMockDocsForTableMutations(14, [
      ['A1', 'B1'],
      ['A2', 'B2'],
    ]);

    await mergeTableCells(mockDocs as any, 'doc123', 14, 0, 0, 2, 2);
  ...
  it('should reject mergeTableCells when the requested span exceeds the table bounds', async () => {
    const mockDocs = buildMockDocsForTableMutations(14, [['A1', 'B1']]);

    await expect(mergeTableCells(mockDocs as any, 'doc123', 14, 0, 1, 1, 2)).rejects.toThrow(
      'columnSpan 2 from columnIndex 1 exceeds table width 2.'
    );

    expect(mockDocs.documents.batchUpdate).not.toHaveBeenCalled();
  });

  it('should create an updateTableCellStyle request with the requested background color', async () => {
    const mockDocs = buildMockDocsForTableMutations(22, [
      ['A1', 'B1'],
      ['A2', 'B2'],
    ]);

    await updateTableCellBackgroundColor(mockDocs as any, 'doc123', 22, 1, 0, 1, 2, '#FFCC00');
  ...
  it('should support short hex colors in updateTableCellBackgroundColor', async () => {
    const mockDocs = buildMockDocsForTableMutations(22, [['A1']]);

    await updateTableCellBackgroundColor(mockDocs as any, 'doc123', 22, 0, 0, 1, 1, '#FC0');
