...
async function getTableBounds(
  ...
  const tableElement = bodyContent.find((el) => el.table && el.startIndex === tableStartIndex);
  if (!tableElement?.table?.tableRows?.length) {
    throw new UserError(
      `No table found at startIndex ${tableStartIndex}. Use readDocument with format='json' to inspect the table indices.`
    );
  }

  const rowCount = tableElement.table.tableRows.length;
  const maxColumnCount = Math.max(
    ...tableElement.table.tableRows.map((row) => row.tableCells?.length ?? 0)
  );

  return { rowCount, maxColumnCount };
}

export async function validateTableCellSpan(
  docs: Docs,
  documentId: string,
  tableStartIndex: number,
  rowIndex: number,
  ...
  const { rowCount, maxColumnCount } = await getTableBounds(docs, documentId, tableStartIndex, tabId);

  if (rowIndex >= rowCount) {
    throw new UserError(
      `Row index ${rowIndex} is out of range. Table has ${rowCount} rows (0-based).`
    );
  }
  if (columnIndex >= maxColumnCount) {
    throw new UserError(
      `Column index ${columnIndex} is out of range. Table has up to ${maxColumnCount} columns (0-based).`
    );
  }
  if (rowIndex + rowSpan > rowCount) {
    throw new UserError(
      `rowSpan ${rowSpan} from rowIndex ${rowIndex} exceeds table row count ${rowCount}.`
    );
  }
  if (columnIndex + columnSpan > maxColumnCount) {
    throw new UserError(
      `columnSpan ${columnSpan} from columnIndex ${columnIndex} exceeds table width ${maxColumnCount}.`
    );
  }
}

function buildTableRange(
  tableStartIndex: number,
  rowIndex: number,
  columnIndex: number,
  rowSpan: number,
...
export async function mergeTableCells(
  ...
  if (rowSpan === 1 && columnSpan === 1) {
    throw new UserError('mergeTableCells requires rowSpan or columnSpan greater than 1.');
  }

  await validateTableCellSpan(
    docs,
    documentId,
    tableStartIndex,
    rowIndex,
...
export async function updateTableCellBackgroundColor(
  docs: Docs,
  documentId: string,
  tableStartIndex: number,
  rowIndex: number,
  ...
  const rgbColor = hexToRgbColor(backgroundColor);
  if (!rgbColor) {
    throw new UserError(`Invalid background hex color format: ${backgroundColor}`);
  }
