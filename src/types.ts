...
export const TableCellLocationParameters = z.object({
  tableStartIndex: z
    .number()
    .int()
    .min(1)
    .describe(
      "The startIndex of the table structural element. Use readDocument with format='json' to inspect table indices."
    ),
  rowIndex: z
    .number()
    .int()
    .min(0)
    .describe('Zero-based row index of the starting cell.'),
  columnIndex: z
    .number()
    .int()
    .min(0)
    .describe('Zero-based column index of the starting cell.'),
});

export const TableCellRangeParameters = TableCellLocationParameters.extend({
  rowSpan: z
    .number()
    .int()
    .min(1)
    .default(1)
    .describe('Number of rows covered by the operation, starting from rowIndex.'),
  columnSpan: z
    .number()
    .int()
    .min(1)
    .default(1)
    .describe('Number of columns covered by the operation, starting from columnIndex.'),
});
