...
```bash
git clone https://github.com/a-bonus/scd-mcp-docs.git
cd scd-mcp-docs
npm install
npm run build
```
...
Both options save the refresh token to `~/.config/scd-mcp-docs/token.json`.
...
Tokens are persisted to `~/.config/scd-mcp-docs/token.json` (respects `XDG_CONFIG_HOME`).
...
The npm package is published as `scd-mcp-docs`.
